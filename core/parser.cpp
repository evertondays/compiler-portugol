...
std::vector<Instruction> init_instructions() {
    ...
    instruction.tokens = {"MATH_OPERATION_OR_NUM", "OPMAIS",
                          "MATH_OPERATION_OR_NUM"};
    instruction.make = "MATH_OPERATION";
    instruction.is_final = false;
    instructions.push_back(instruction);

    instruction.tokens = {"MATH_OPERATION_OR_NUM", "OPMENOS",
                          "MATH_OPERATION_OR_NUM"};
    instruction.make = "MATH_OPERATION";
    instruction.is_final = false;
    instructions.push_back(instruction);

    instruction.tokens = {"MATH_OPERATION_OR_NUM", "OPDIVI",
                          "MATH_OPERATION_OR_NUM"};
    instruction.make = "MATH_OPERATION";
    instruction.is_final = false;
    instructions.push_back(instruction);

    instruction.tokens = {"MATH_OPERATION_OR_NUM", "OPMULTI",
                          "MATH_OPERATION_OR_NUM"};
    instruction.make = "MATH_OPERATION";
    instruction.is_final = false;
    instructions.push_back(instruction);
    ...
    // se
    instruction.tokens = {"SE",    "LOGIC_OPERATION", "ENTAO", "INSTRUCTIONS",
                          "SENAO", "INSTRUCTIONS",    "FIMSE"};
    instruction.is_final = true;
    instruction.make = "FINAL";
    instructions.push_back(instruction);
    ...
    // para
    instruction.tokens = {"PARA",  "ID_OR_NUM", "ATE",          "ID_OR_NUM",
                          "PASSO", "ID_OR_NUM", "INSTRUCTIONS", "FIMPARA"};
    instruction.is_final = true;
    instruction.make = "FINAL";
    instructions.push_back(instruction);

    instruction.tokens = {"PARA",      "ID_OR_NUM",    "ATE",
                          "ID_OR_NUM", "INSTRUCTIONS", "FIMPARA"};
    instruction.is_final = true;
    instruction.make = "FINAL";
    instructions.push_back(instruction);
...
bool tokens_are_equal(std::string queue_token, std::string instruction_token) {
    if (instruction_token == "ID_OR_NUM") {
        return queue_token == "ID" || queue_token == "NUMINT" ||
               queue_token == "MATH_OPERATION";
    }

    if (instruction_token == "LOGIC_TOKEN") {
        return queue_token == "LOGIGUAL" || queue_token == "LOGMAIOR" ||
               queue_token == "LOGMAIORIGUAL" ||
               queue_token == "LOGMAIORIGUAL" || queue_token == "LOGMENOR" ||
               queue_token == "LOGMENORIGUAL" || queue_token == "LOGDIFF" ||
               queue_token == "OU" || queue_token == "E";
    }
...
ReduceResult try_reduce(int token_index, const std::vector<Token>& queue,
    ...
    for (const auto& instruction : instructions) {
        ...
        if (token_index - tokens_to_check + 1 < 0)
            continue;

        for (int i = 0; i < tokens_to_check; ++i) {
            if (!tokens_are_equal(queue[token_index - i].type,
    ...
    while (!tokens.empty() || reduced) {
        ...
        for (int i = queue.size() - 1; i >= 0; --i) {
            ...
            if (reduce_result.success) {
                ...
                for (int j = i - reduce_result.quantity_of_reduce_tokens + 1;
                     j <= i; ++j) {
                ...
                log_file << "Reduzindo tokens: [" << reduced_types << "] -> "
                         << reduced_token.type << "\n";

                queue.erase(
                    queue.begin() +
                        (i - reduce_result.quantity_of_reduce_tokens + 1),
                    queue.begin() + i + 1);
                queue.insert(
                    queue.begin() +
                        (i - reduce_result.quantity_of_reduce_tokens + 1),
...
bool validate_parser(std::vector<Token> queue) {
    ...
    std::cout << ANSI_COLOR_RED << std::endl
              << "Sintaxe inválida!" << std::endl;
    std::cout
        << "Você pode verificar os logs dentro da pasta 'build' para debug."
